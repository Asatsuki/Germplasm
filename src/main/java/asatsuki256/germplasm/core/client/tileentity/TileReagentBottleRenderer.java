...
public class TileReagentBottleRenderer extends TileEntitySpecialRenderer<TileReagentBottle>{
	
	@Override
	public void render(TileReagentBottle tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
		...
		for(int i = 0; i < 4; i++) {
			if (tile.getFluid(i) != null) {
				renderFluid(tessellator, tile.getFluid(i), i);
			}
		}
	...
	private void renderFluid(Tessellator tessellator, FluidStack fluid, int bottle) {
		...
		float f = 3f - offset;
		float bottom = offset;
		float top = offset + (10f - offset*2) * (float)fluid.amount / (float)TileReagentBottle.CAPACITY;
